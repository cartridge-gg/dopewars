...
mod travel {
    ...
    fn execute(ctx: Context, game_id: u32, next_location_id: felt252) -> bool {
        ...
        if event_occured {
            emit!(ctx.world, RandomEvent { game_id, player_id, health_loss: result.health_loss, mugged: result.mugged, arrested: result.arrested });
        }
        ...
        emit!(ctx.world, Traveled { game_id, player_id, from_location: player.location_id, to_location: next_location_id });
