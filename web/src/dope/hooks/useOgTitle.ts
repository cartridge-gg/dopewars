...
export const useOgTitle = (tokenId: number) => {
  ...
  const namePrefixesValues = getComponentValuesBySlug(
    "DopeGear",
    "Name Prefixes"
  );
  const nameSuffixesValues = getComponentValuesBySlug(
    "DopeGear",
    "Name Suffixes"
  );

  const rand = BigInt(
    keccak256(encodePacked(["string", "uint256"], ["OG", BigInt(tokenId)]))
  );
  // console.log("encodePacked", encodePacked(["string", "uint256"], ["OG", BigInt(tokenId)]))

  const prefix =
    namePrefixesValues[
      Number((rand % BigInt(namePrefixesValues.length - 1 - 2)) + 1n)
    ].value;

  const suffix =
    nameSuffixesValues[
      Number((rand % BigInt(nameSuffixesValues.length - 1)) + 1n)
    ].value;
