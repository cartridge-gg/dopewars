...
export const usePlayerState = create<PlayerState>((set) => ({
  ...
  addTrade: (drug: Drugs, trade: TradeType) =>
    set((state) => {
      ...
      if (existingTrade.direction === trade.direction) {
        quantity += trade.quantity;
      } else {
        // if the existing trade has the opposite direction, subtract quantities
        quantity -= trade.quantity;

        // if newQuantity < 0, reverse the direction and make the quantity positive
        if (quantity < 0) {
          quantity = -quantity;
          direction = trade.direction;
        }

        // if newQuantity == 0, remove the trade
        if (quantity === 0) {
      ...
      // If newQuantity > 0, update the trade
