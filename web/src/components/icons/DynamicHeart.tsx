...
export const DynamicHeart = (props: IconProps & { health: number; maxHealth: number; color: string }) => {
  ...
  const { health, maxHealth, color, ...rest } = props;
  const healthWidth = ((health * 4) / maxHealth) * 24;
  const gradientId = `health-gradient-${health}`;
      ...
      <>
        <defs>
          <linearGradient id={gradientId} x1="0%" y1="0%" x2="100%" y2="0%">
            <stop offset={`${healthWidth}%`} stopColor={color} />
