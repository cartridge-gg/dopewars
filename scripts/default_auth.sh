...
export MINI_GAME=$(cat ./target/dev/manifest.json | jq -r '.contracts[] | select(.name == "minigame" ).address')
...
MINI_GAME_COMPONENTS=("Player" "Game" "Encounter" "Tile" "PlayerMiniGame" "Gangster" "Cop" "Direction" "Map")
...
for component in ${MINI_GAME_COMPONENTS[@]}; do
    sozo -P $PROFILE auth writer $component $MINI_GAME --world $WORLD_ADDRESS 
    sleep 0.1
done
